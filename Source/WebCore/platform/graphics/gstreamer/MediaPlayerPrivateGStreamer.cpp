...
GstPadProbeReturn MediaPlayerPrivateGStreamer::streamStartProbeCallback(GstPad*, GstPadProbeInfo* info, gpointer data)
{
    ...
    auto mediaPlayer = probeData->player->m_player;

    for (auto& track : audioTracks.values()) {
        if (track->trackIndex() == probeData->trackIndex) {
            mediaPlayer->removeAudioTrack(*track);
            track->setId(streamId);
            mediaPlayer->addAudioTrack(*track);
            mediaPlayer->mediaEngineUpdated();
...
void MediaPlayerPrivateGStreamer::notifyPlayerOfAudio()
{
    ...
    for (unsigned i = 0; i < numTracks; ++i) {
        ...
        auto track = AudioTrackPrivateGStreamer::create(makeWeakPtr(*this), i, pad, streamId);
        if (!track->trackIndex())
            track->setActive(true);
        ASSERT(streamId == track->id());
