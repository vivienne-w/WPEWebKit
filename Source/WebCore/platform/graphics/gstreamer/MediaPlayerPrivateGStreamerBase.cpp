...
static void installOrUninstallProbeIfNeeded(MediaPlayerPrivateGStreamerBase* player, const char* elementName, const char* padName, GstState installState, uint debugProbeId, GstMessage* message)
{
    if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_STATE_CHANGED
        && g_strstr_len(GST_MESSAGE_SRC_NAME(message), -1, elementName)) {
        gulong& probeId = player->debugProbeIds[debugProbeId];
...
bool MediaPlayerPrivateGStreamerBase::handleSyncMessage(GstMessage* message)
{
    ...
    /*
    if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_STATE_CHANGED) {
        GstState currentState, newState, pendingState;
    ...
    */
    ...
    //installOrUninstallProbeIfNeeded(this, "webkit-gl-video-sink", "sink", GST_STATE_READY, 1, message);
    //installOrUninstallProbeIfNeeded(this, "omxh264dec", "sink", GST_STATE_READY, 2, message);
    //installOrUninstallProbeIfNeeded(this, "omxh264dec", "src", GST_STATE_READY, 3, message);
    //installOrUninstallProbeIfNeeded(this, "autoaudiosink1-actual-sink-omxhdmiaudio", "sink", GST_STATE_PAUSED, 4, message);
    installOrUninstallProbeIfNeeded(this, "brcmvidfilter", "brcm-vidfilter-sink", GST_STATE_READY, 1, message);
    installOrUninstallProbeIfNeeded(this, "brcmvideodecoder", "sink", GST_STATE_READY, 2, message);
    installOrUninstallProbeIfNeeded(this, "brcmvideodecoder", "brcm-video-src", GST_STATE_READY, 3, message);
