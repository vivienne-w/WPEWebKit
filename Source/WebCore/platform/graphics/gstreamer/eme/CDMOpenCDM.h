...
public:
    ...
    // FIXME: Use these to pass information back to JS about
    // changed keys, etc.
    void setClient(WeakPtr<CDMInstanceSessionClient>&&) { }
    void clearClient() { }

    void requestLicense(LicenseType, const AtomicString& initDataType, Ref<SharedBuffer>&& initData, LicenseCallback&&) final;
    void updateLicense(const String& sessionId, LicenseType, const SharedBuffer& response, LicenseUpdateCallback&&) final;
