...
void CDMInstanceOpenCDM::requestLicense(LicenseType licenseType, const AtomicString&, Ref<SharedBuffer>&& initData, LicenseCallback callback)
{
    ...
    RefPtr<Session> newSession = adoptRef(*new Session(openCDM, WTFMove(initData)));
    if (!addSession(sessionIdValue, newSession)) {
        GST_WARNING("Failed to add session %s", sessionId.c_str());
        return;
    }
...
String CDMInstanceOpenCDM::sessionIdByInitData(const InitData& initData, bool firstInLine) const
{
    ...
    for (const auto& pair : m_sessionsMap) {
        ...
        const RefPtr<Session>& session = pair.second;
...
bool CDMInstanceOpenCDM::isSessionIdUsable(const String& sessionId) const
{
    ...
    return session->lastStatus() == media::OpenCdm::KeyStatus::Usable;
}
...
bool CDMInstanceOpenCDM::addSession(const String& sessionId, RefPtr<Session> session)
{
    ...
    std::string sessionIdAsString(sessionId.utf8().data());
    auto insertResult = m_sessionsMap.insert({sessionIdAsString, session});
...
bool CDMInstanceOpenCDM::removeSession(const String& sessionId)
{
    ...
    std::string sessionIdAsString(sessionId.utf8().data());
    return m_sessionsMap.erase(sessionIdAsString) > 0;
}
...
RefPtr<CDMInstanceOpenCDM::Session> CDMInstanceOpenCDM::lookupSession(const String& sessionId) const
{
    ...
    std::string sessionIdAsString(sessionId.utf8().data());
    auto iterator = m_sessionsMap.find(sessionIdAsString);
