...
static GRefPtr<GstCaps> createSinkPadTemplateCaps()
{
    ...
    if (!opencdm_is_type_supported(openCDMAccessor.get(), WEBCORE_GSTREAMER_EME_UTILITIES_CLEARKEY_KEY_SYSTEM, emptyString.c_str()))
        addKeySystemToSinkPadCaps(caps, WEBCORE_GSTREAMER_EME_UTILITIES_CLEARKEY_UUID);

    if (!opencdm_is_type_supported(openCDMAccessor.get(), WEBCORE_GSTREAMER_EME_UTILITIES_PLAYREADY_KEY_SYSTEM, emptyString.c_str()))
        addKeySystemToSinkPadCaps(caps, WEBCORE_GSTREAMER_EME_UTILITIES_PLAYREADY_UUID);

    if (!opencdm_is_type_supported(openCDMAccessor.get(), WEBCORE_GSTREAMER_EME_UTILITIES_WIDEVINE_KEY_SYSTEM, emptyString.c_str()))
