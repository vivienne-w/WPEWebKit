...
static bool webKitMediaOpenCDMDecryptorHandleInitData(WebKitMediaCommonEncryptionDecrypt* self, const WebCore::InitData&);
...
static GRefPtr<GstCaps> createSinkPadTemplateCaps()
{
    ...
    if (openCdm.IsTypeSupported(WEBCORE_GSTREAMER_EME_UTILITIES_CLEARKEY_KEY_SYSTEM, emptyString))
        addKeySystemToSinkPadCaps(caps, WEBCORE_GSTREAMER_EME_UTILITIES_CLEARKEY_UUID);

    if (openCdm.IsTypeSupported(WEBCORE_GSTREAMER_EME_UTILITIES_PLAYREADY_KEY_SYSTEM, emptyString))
        addKeySystemToSinkPadCaps(caps, WEBCORE_GSTREAMER_EME_UTILITIES_PLAYREADY_UUID);

    if (openCdm.IsTypeSupported(WEBCORE_GSTREAMER_EME_UTILITIES_WIDEVINE_KEY_SYSTEM, emptyString))
...
static SessionResult webKitMediaOpenCDMDecryptorResetSessionFromInitDataIfNeeded(WebKitMediaCommonEncryptionDecrypt* self, const WebCore::InitData& initData)
{
    ...
    RefPtr<WebCore::CDMInstance> cdmInstance = getCDMInstance(self);
...
{
    GST_TRACE_OBJECT(self, "handling init data of size %u and MD5 %s", initData.size(), initDataMD5(initData).utf8().data());
...
{
    GST_TRACE_OBJECT(self, "attempting to decrypt with local instance and init data of size %u and MD5 %s", initData.size(), initDataMD5(initData).utf8().data());
    return webKitMediaOpenCDMDecryptorResetSessionFromInitDataIfNeeded(self, initData) != InvalidSession;
}
...
static bool webKitMediaOpenCDMDecryptorDecrypt(WebKitMediaCommonEncryptionDecrypt* self, GstBuffer* keyIDBuffer, GstBuffer* ivBuffer, GstBuffer* buffer, unsigned subSampleCount, GstBuffer* subSamplesBuffer)
{
    ...
    auto mappedIV = GstMappedBuffer::create(ivBuffer, GST_MAP_READ);
    if (!mappedIV) {
    ...
    auto mappedBuffer = GstMappedBuffer::create(buffer, GST_MAP_READWRITE);
    if (!mappedBuffer) {
        GST_ERROR_OBJECT(self, "Failed to map buffer");
        return false;
    }

    auto mappedKeyID = GstMappedBuffer::create(keyIDBuffer, GST_MAP_READ);
    if (!mappedKeyID) {
    ...
    int errorCode;
    if (subSamplesBuffer) {
        auto mappedSubSamples = GstMappedBuffer::create(subSamplesBuffer, GST_MAP_READ);
        if (!mappedSubSamples) {
